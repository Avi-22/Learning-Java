/**
 * This class represents a real polynomial on a single variable 'x'.
 * 
 * @author Into2CS team @idc.ac.il
 */
public class Polynomial {
	...
	/**
	 * A standard constructor. Ensures that the highest term of the polynomial
	 * is non-zero.
	 * 
	 * @param coefficients
	 *            - a list of the polynomial's coefficients
	 * @throws RuntimeException
	 *             in the case no coefficients are entered.
	 */
	...
	/**
	 * Returns the degree of the polynomial. The 0 polynomial is considered to
	 * have degree 0.
	 * 
	...
	public double evaluate(double x) {
		double ans = 0;
		for (int i = 0; i <= this.degree(); i = i + 1) { // sum all parts of the
	 ...
	 * 
	 * @param other
	 *            - the polynomial to be added to this
	 ...
	 * 
	 * @param other
	 *            - the polynomial to be multiplied with this
		...
		for (int i = 0; i < products.length; i++) {
			for (int j = 0; j <= i; j++) {
				if (j <= this.degree() && (i - j) <= other.degree()) {
					products[i] += this.coefficients[j]
							* other.getCoefficients()[i - j];
				}
			}
		}
	 ...
	 * 
	 * @param other
	 *            - the polynomial to be subtracted from this
	 ...
	 * 
	 * @param other
	 * @return true if and only if other is a polynomial whose coefficients are
	 *         equal to this polynomial's.
	 */
